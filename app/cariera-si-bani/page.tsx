...
export default function CarieraSimbanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/career-finance/calculate', {
        method: 'POST',
      ...
      if (data.report.guestToken) {
        router.push(`/cariera-si-bani/raport/${data.report.id}?token=${data.report.guestToken}`)
      } else {
        router.push(`/cariera-si-bani/raport/${data.report.id}`)
      }
    } catch (err) {
        ...
        >
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-300 mb-2">
              Descoperă potențialul astrologic pentru succes profesional și prosperitate financiară
            </p>
            <p className="text-sm text-gray-400">
              Raport personalizat bazat pe harta ta natală
            </p>
          </div>
          ...
          >
            {[
              { title: 'Analiză Profundă', desc: '8 indicatori astrologici pentru carieră' },
              { title: '20+ Secțiuni', desc: 'Raport complet în limba română' },
              { title: 'Dashboard Interactiv', desc: 'Grafice și timeline personalizate' },
            ].map((item, i) => (
