...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await saveRaportAndSendEmail(session.id, { ...formData, facet: metadata.facet, email, first: formData.first ?? "", last: formData.last ?? "", middle: formData.middle ?? "" }, "ru", "grani")
