      ...
      <div className="mx-auto max-w-[1120px]">
        ...
        <nav ref={toolbarRef} id="prima-cheie" aria-label={ro ? 'Navigare raport și calcul personal' : 'Разделы и личный расчёт'} className="sticky top-[env(safe-area-inset-top,0px)] z-30 scroll-mt-0 border border-primary/20 bg-card/95 p-2 text-foreground shadow-lg backdrop-blur-xl sm:rounded-b-xl">
          <div className="flex items-center gap-2">
            <span className="hidden max-w-[260px] shrink-0 px-2 text-sm font-semibold leading-5 text-foreground/85 lg:block">{ro ? 'Introdu datele tale personale și apasă butonul galben pentru calcularea raportului.' : 'Введи свои личные данные и нажми жёлтую кнопку, чтобы рассчитать персональный разбор.'}{chapters.length > 0 && ` · ${chapters.length}`}</span>
