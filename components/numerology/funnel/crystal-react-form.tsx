...
export function CrystalReactForm({ initialEmail = '', initialValues, locale = 'ru', variant = 'form-control', futureStage = 'date', onBirthSubmit, onFirstInteraction, onStepComplete, onSubmit }: CrystalReactFormProps) {
  ...
  const dateFields = (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 text-sm font-semibold text-foreground"><CalendarDays className="size-4 text-primary" /><span className="font-mono uppercase tracking-[0.14em]">{c.date}</span></div>
  ...
  return (
    <section className="w-full min-w-0 overflow-hidden rounded-2xl border border-border bg-card/60 shadow-2xl shadow-background/40">
      <header className="flex flex-col gap-4 border-b border-border bg-card/70 px-5 py-6 sm:px-8">
        ...
        <div className="flex items-start gap-4"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-primary/40 bg-primary/10 text-primary"><Icon className="size-5" /></div><div><h2 className="text-balance text-xl font-bold tracking-tight text-foreground sm:text-2xl">{title}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-foreground/80">{body}</p></div></div>
